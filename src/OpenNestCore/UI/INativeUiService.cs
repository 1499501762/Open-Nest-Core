...
///
/// 本接口在 Core 定义契约（仅依赖 UnityEngine 与 TMP，不依赖游戏 Assembly-CSharp）；
/// 游戏侧实现类（能引用 Assembly-CSharp 的模组，如 OpenNestCoop 的 IronNestNativeUi）实现并注册
/// 到 <see cref="NativeUi"/>。未注册时所有门面方法安全返回默认值（null / false / original）。
/// </summary>
...
/// <summary>
/// 游戏原生 UI 桥接注册表 + 静态门面。任何模组调用 <c>NativeUi.xxx</c> 即可使用游戏原生 UI 能力，
